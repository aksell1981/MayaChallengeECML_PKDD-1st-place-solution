...
os.makedirs(cfg.out_dir+'/models', exist_ok=True)
os.makedirs(cfg.out_dir+'/logs', exist_ok=True)
...
#some more things
logger=CSVLogger(['epoch','train_loss','val_loss','dice_coef'],f"{cfg.out_dir}/logs/log_fold{cfg.fold}_{cfg.prefix}.csv")
#metric=Dice_soft()
metric=Dice_th()
best_metric=0
...
for epoch in range(cfg.epochs):
    ...
    for data in tqdm(dataloader):
        ...
        loss=symmetric_lovasz(output,mask)
    ...
    if metric_this_epoch>best_metric:
        torch.save(model.state_dict(),f'{cfg.out_dir}/models/{cfg.encoder}_fold{cfg.fold}_{cfg.prefix}.pth')
